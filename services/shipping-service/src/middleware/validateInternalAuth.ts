...
// Same contract as order-service / inventory-service: validates the INTERNAL
// JWT minted by the api-gateway (different secret than customer access
// tokens, issuer + audience pinned). All traffic must come through the gateway.
...
// RBAC: mutating ops (manual dispatch, status updates) are admin-only.
